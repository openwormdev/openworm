...
The dashboard has no third-party scripts, trackers, external fonts, or automatic wallet discovery. Its only automatic reads are same-origin static assets. A separately configured worker receives only a built-in scenario name; the browser never uploads imported reports. Worker tokens stay in memory and are never included in exports or browser storage. The browser rejects credential-bearing URLs, insecure remote HTTP, and redirects when sending a token.
